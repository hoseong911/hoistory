...
### 글자가 못 들어가는 곳 (`BLOCKED`)

**면류관과 옷깃만 막고 나머지는 머리 안이든 밖이든 다 쓴다.** `brain.js`의 `BLOCKED`는 64x64 칸(한 칸 = VB 5)짜리 표이고 눈대중이 아니라 그림을 캔버스로 읽어 만든 값이다.

1. 흰 배경이 아닌 화소를 모은다(선과 면이 모두 들어온다).
2. **열기**(반지름 6으로 깎았다 부풀리기)로 가는 선을 지운다 — 머리 윤곽선과 목선은 사라지고 면류관, 유(구슬 술), 옷깃 같은 덩어리만 남는다. 이 한 단계가 핵심이다.
3. 글자가 닿지 않게 반지름 9로 한 번 더 부풀린 뒤 칸으로 줄인다.

**그림을 바꾸면 이 표를 다시 재야 한다.** 예전에 타원 하나(`FIELD_ROWS`)로 잡던 방식은 버렸다 — 면류관이 왼쪽 위를 비스듬히 덮고 있어 타원으로는 머릿속만 겨우 쓰게 되고 그림의 절반이 놀았다.

### 구름과 자리 잡기

- 키워드는 네모 상자가 아니라 **구름**이다(`cloudPath`). 타원 위에 점을 고르게 찍고 이웃한 두 점을 바깥으로 불룩한 호로 이어 가리비 모양을 만든다. 네모에 혹을 붙이는 방식으로는 테두리를 길 하나로 그릴 수 없어 실선/점선을 가를 수 없다.
- **테두리가 곧 상태 표시다. 점선 = 까닭을 아직 안 씀, 실선 = 씀**(`.bm-open`). 예전에 칩 밑에 점을 찍던 표시(`bm-need`)는 없앴다.
- 구름은 글자 네모를 품어야 하므로 가로 1.44배, 세로 1.56배로 잡는다(네모의 모서리가 타원 안에 들어오는 값).
- **`settle()`이 그린 뒤에 자리를 잡는다.** 글자 상자를 실제로 재서 구름을 씌우고, 막힌 데에 걸쳤거나 이미 놓인 구름과 겹치면 `findSpot()`이 나선으로 가장 가까운 빈자리를 찾는다. 낱말 길이와 비중에 따라 구름 크기가 제각각이라 놓을 때 좌표만 보고는 알 수 없기 때문이다. 갤러리와 어드민에서도 돌아가므로 예전 좌표로 저장된 작품도 제자리에 들어와 보인다(저장값은 건드리지 않고 화면만 고친다).
- 끌어 옮길 때도 같은 표를 본다. 막힌 데에 닿으면 한 축씩 따로 시도해 **가장자리를 따라 미끄러진다**(면류관 위로는 끌려 들어가지 않는다).
- 임금 이름표(`.bm-name`)를 옷깃 위에 두는 것은 우연이 아니다 — 거기가 막힌 칸이라 구름이 절대 오지 않는다.

### 크기

- 무대는 최대 560px, 칩 글자 크기는 `fitFont()`가 무대 폭에 비례해 정한다. 그래서 **폰에서 그림이 작아져도 들어가는 키워드 수는 데스크톱과 같다** — 글자가 같은 비율로 줄기 때문이다.
- 한 인물당 기본 8칸(`DEFAULT_CONFIG.maxChips`). 어드민 SETTING에서 조절한다.
- 두 임금은 **같은 그림 한 장**을 쓰고 색(`--bm-yeongjo` / `--bm-jeongjo`)과 이름표로만 갈라진다. 색을 입히려 CSS filter를 걸면 관의 금색과 붉은 깃까지 같이 돌아가 딴 그림이 된다.
- 구름 색도 임금별로 갈린다. `brain.js`가 무대에 `.bm-side-<임금>` 을 붙이고 거기서 `--bm-cloud-fill`(연한 채움) / `--bm-cloud-line`(진한 테두리) / `--bm-cloud-sel`(고른 것)을 정한다. **양쪽 HTML에 구름 색을 따로 적지 말 것** — 학생 화면과 어드민이 어긋난다.
- 머리 그림 바탕이 흰색이라 그림을 담는 칸(학생 무대, 갤러리 카드, 크게 보기, 어드민 미리보기)은 모두 **흰색**이다. 반투명 바탕을 쓰면 그림의 네모가 드러난다.
- 학생 화면은 `hi-preset-paper` 를 쓰되 `--hi-*` 색을 body 에서 먹빛 남색으로 덮어쓴다(프리셋의 금색이 흰 그림 옆에서 바랜 종이처럼 보였다). **`shared/theme.css` 를 고치지 말 것** — 같은 프리셋을 다른 앱도 쓴다.

### 설정 (`settings/j_brain_config`)

- `intro` — 활동 첫 화면 안내글. 어드민 SETTING의 [활동 안내글]에서 고치고, **빈 줄 하나가 문단 구분**이다. 기본값은 `brain.js`의 `DEFAULT_INTRO`. 맨 아래 "두 임금의 머릿속을 채워 보세요" 한 줄은 `index.html`에 붙박이라 이 글에 없다.
- `aiPrompt` — 까닭 자동 검토가 보는 눈. 어드민 SETTING의 [까닭 자동 검토 기준]에서 고친다. 기본값은 `brain.js`의 `DEFAULT_AI_PROMPT`.
- `keywords`, `maxChips`, `minChips` — 예전 그대로.
- **까닭은 글자 수를 제한하지 않는다.** 세는 칸도 안내 문구도 없고, 칸이 글에 맞춰 자란다. 저장할 때만 `NOTE_HARD_CAP`(1000자)으로 조용히 끊는다. 예전 `noteMax` 설정은 없앴다 — 되살리지 말 것.

### 채점: 키워드는 자동, 까닭은 AI 보조 + 선생님 확정

두 셈이 **따로 돌아간다**. 섞지 말 것.

- **[자동 채점]** — 키워드를 맞게 골랐는지만 본다(`judge()` → 맞음/다름/함정). 함정 0개 + 정답률이 임계값 이상 + 까닭을 전부 씀 이면 `status: 'pass'`. **이미 선생님이 눌러 둔 학생은 건드리지 않는다.**
- **[까닭 자동 검토]** — 학생 한 명을 `claudeProxy`로 한 번 부르고(haiku-4-5), `aiReview: {level, comment, weak, at}` 를 작품 문서에 쓴다. `level` 은 `good`/`mid`/`weak` 세 값이고 화면에는 충실·보통·부실로 뜬다. **`status` 에는 절대 손대지 않는다** — 통과·미흡은 선생님이 누른 것만 기록이다.
  - 한 명을 한 번에 보내는 이유: "크기와 까닭이 어울리는가"는 한 사람의 두 임금을 같이 봐야 따질 수 있다.
  - 이미 본 사람은 다시 부르지 않는다. **검토한 뒤에 학생이 다시 냈으면**(`submittedAt > aiReview.at`) stale 로 보아 다시 검토 대상이 되고, 화면에는 "AI 충실 · 수정됨" 처럼 회색 배지로 뜬다.
  - 셋씩 나란히 부른다(`worker()` 세 개). 한 명씩이면 한 반에 너무 오래 걸리고, 한꺼번에 다 부르면 막힌다.
  - 키워드 정오는 프롬프트에서 "정답표로 이미 따로 세고 있으니 판정을 걸지 말라"고 못박아 둔다. 안 그러면 같은 잘못이 두 번 깎인다.
- `j_brain_works` 규칙에서 **`status` 와 `aiReview` 는 어드민만 쓴다.** 학생이 다시 제출해도 그 두 칸은 못 건드린다(`diff().affectedKeys().hasAny([...])`).
...
## 네컷 작품은 그림을 따로 저장한다 (2026-08-31)

`fourcut_works/{학번}`은 **메타데이터만** 두고(약 1.5KB), 그림 본문은 `fourcut_images/{학번}`에 따로 넣는다. 문서 id가 같아서 짝이 저절로 맞는다.

- **왜**: 갤러리와 어드민은 목록을 만들려고 작품 문서를 통째로 받는다. 그림이 같이 들어 있던 시절엔 한 건이 752KB(그중 99.8%가 이미지)라 160명이면 117MB가 오갔다. **`fourcut_works`에 `image` 필드를 다시 넣지 말 것.**
- 그림은 **실제로 보는 것만** 가져온다. 학생 갤러리는 지금 보는 한 장과 다음 한 장, 어드민 목록은 화면에 들어온 카드만(IntersectionObserver, `rootMargin:400px`), 한 명씩 보기는 지금 사람과 다음 사람. 받은 것은 `workImg` 캐시에 담아 다시 받지 않는다.
- 그림이 문서 안에 있던 시절의 작품도 그대로 열린다 — `loadWorkImage()`가 `w.image`가 있으면 그걸 먼저 쓴다.
- 제출은 **그림 먼저, 목록 문서 나중**이다. 반대로 하면 목록에는 떴는데 그림이 없는 순간이 생긴다. 삭제는 두 문서를 같이 지운다.
- 완성 그림은 두 벌 만든다 — 학생이 내려받는 원본(1080px, 품질 0.85)과 저장본(800px, 품질 0.72). 화면에서 보는 데는 800px이면 충분하다(`composeImage()` / `shrinkDataUrl()`).
...
- 페이지(슬라이드) 단위 예외는 콘텐츠 편집의 "형식 변경 / 페이지 설정"에 있는 글자 크기 입력칸이며, `renderSlideHTML`이 그 슬라이드에만 인라인 스타일로 변수를 덮어쓴다.
  - **덮어쓸 변수는 `PAGE_FS_VARS`(`renderSlideHTML` 바로 위) 한 표에 형식별로 적혀 있다.** 형식마다 본문이 읽는 변수가 다르기 때문이다(행 나열 `--fs-rows-body`, 사료 인용 `--fs-qt-text`, 연표 `--fs-tl-text` …). `--fs-body`도 함께 남기지만 그건 스펙에 아직 안 올라온 규칙을 위한 안전판일 뿐이다.
  - 2026-09-21 이전에는 `--fs-body` 하나만 덮어썼다. 디자인 탭이 형식별 변수를 **언제나** 채우기 때문에 `--fs-body`는 어느 규칙에도 닿지 못했고, 그래서 **중앙 나열을 뺀 모든 형식에서 페이지별 글자 크기가 통째로 먹지 않았다**(중앙 나열만 따로 손봐 둔 상태였다). 새 형식을 만들면 이 표에 한 줄을 더해야 그 형식에서도 먹는다.
