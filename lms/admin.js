...
function cePopulateLessonSelect() {
  ...
  sel.innerHTML = '<option value="">— 강의 선택 —</option>' +
    [...ceLessonsCache].reverse().map(l => `<option value="${esc(l.num)}" ${cur && l.num===cur?'selected':''}>${esc(l.num)}강. ${esc(l.title)}</option>`).join('');
}
...
const CE_LESSON_JSON_SCHEMA = `{
  ...
  "conceptContentLines": [
    ...
    { "type": "divider", "title": "슬라이드 제목", "format": "quote", "quoteText": "사료 원문 텍스트 (빈칸은 {정답} 표기)", "quoteSource": "출처 (예: 「삼국사기」)" }
  ],
  "missionContentLines": [
    { "type": "divider", "title": "슬라이드 제목" },
    { "type": "row", "label": "회색 칸(행 라벨) 텍스트", "items": ["원문자(①②③) 단위 항목. 하위 a.b.c. 줄은 <br>로 이어붙임", "..."] },
...
/* ════ 생각 체크 어드민 ════ */
{
  ...
  window.thRunGrading = async function() {
    ...
    if (needAi.length) {
      ...
      const prompt = `역사 수업(중학교 3학년)의 질문과 학생 답변입니다. 각 답변이 "질문에 대해 역사적 맥락에 맞추어 제대로 답했는지"를 0~100점으로 평가하세요(중3 수준 기준). 질문과 무관하거나 의미 없는 반복/복붙은 낮은 점수를 주세요.
질문: "${lec.question}"
