...
async function missionAutoDetect(lessonKey) {
  ...
  Object.keys(_gradeRecords).forEach(sid => {
    ...
    const v = missionVerdictFor(sid, classNum, lessonKey, perSource);
    if (!v.blank) out[sid] = { achieved: v.achieved, onTime: v.onTime, at: v.at };
  });

  return { map: out, apps };
}

/* 한 학생의 미션 판정. 강의 하나를 불러올 때(missionAutoDetect)와 전체 갱신
   (gradeRefreshAllLessons)이 반드시 같은 규칙을 써야 해서 여기 한 곳에 모아 둔다.
   perSource = [{ src, byStudent, failed }] — 앱마다 학번별 제출 문서를 담은 것.
   blank가 true면 판정 보류(제출 컬렉션을 읽지 못함)이므로 표를 건드리지 않는다. */
function missionVerdictFor(sid, classNum, lessonKey, perSource) {
  ...
  return { achieved, onTime, at, blank };
}
...
/* ════ 생각 체크 어드민 ════ */
{
  ...
  window.thRunGrading = async function() {
    ...
    if (needAi.length) {
      ...
      const prompt = `역사 수업(중학교 3학년)의 질문에 대한 학생 답변을 채점합니다. 학생 답변은 보통 공백 제외 150자 안팎으로 짧습니다. 길이가 짧다는 이유로 감점하지 마세요. "질문 취지에 맞게 자기 생각을 근거와 함께 썼는가"를 봅니다. 중3 눈높이에서 격려하는 태도로 보되, 점수는 실제 품질에 따라 분명히 구분해서 매기세요. 성의 있게 자기 생각을 쓴 보통 수준의 답은 75~85 구간이 기본입니다. 자기 입장이 분명하고 근거나 구체적 사례를 갖춘 답에는 90점 이상을 주는 데 인색하지 마세요.
- 90~100: 자기 입장이 분명하고 이유·근거나 구체적 사례를 갖춰 설득력이 있음
...
function xpTplRender() {
  ...
  if (!box) return;
  if (!_xpTpls.length) {
    box.innerHTML = '<div class="xp-tpl-empty">저장된 템플릿이 없습니다. 왼쪽에서 경험치와 사유를 정한 뒤 아래 칸에 이름을 붙여 저장하세요.</div>';
    return;
  }
